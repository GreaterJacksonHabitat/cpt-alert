...
(function ($) {
    ...
    function init() {
        ...
        var $alert_containers = $('.gjh-alerts-container');
        ...
        // Close alerts
        $(document).on('click', '.gjh-alert-close', function (e) {

            e.preventDefault();
            close_inset($(this).closest('.gjh-alert'));
            return false;
        });

        // Close popup on clicking outside
        $(document).on('click', '.gjh-alert.pop-up .gjh-alert-container', function () {
            close_popup($(this).closest('.gjh-alert'));
        });

        $(document).on('click', '.gjh-alert-content', function (e) {
            e.stopPropagation();
        });
    }
    ...
    function load_alerts(e) {
        ...
        data.action = 'gjh_alert_get_alerts';

        $.ajax( {
			type : 'POST',
            url : GJH_Alerts.ajaxurl,
            data : data,
            success : function (response) {
                ...
                for (i = 0; i < response['data']['alerts'].length; i++) {
                    ...
                    alert_ID = 'gjh-alert-' + alert['post_ID'];
    ...
    function build_alert(alert, $alert_container) {
        ...
        var $alert = $alert_container.find('.gjh-alert-dummy.' + alert['type']).clone(),
            $alert_button = $alert.find('.gjh-alert-button');

        $alert.attr('id', 'gjh-alert-' + alert['post_ID'])
            .addClass(alert['color'] + '-background')
            .removeClass('gjh-alert-dummy');

        $alert.find('.gjh-alert-text').append(alert['content']);
        $alert.find('.gjh-alert-icon').addClass(alert['icon']);
        ...
        if (alert['user_interaction'] == 'close_button' || alert['user_interaction'] == 'call_to_action') {
            ...
            if (alert['user_interaction'] == 'close_button') {
                ...
                $alert_button.addClass('gjh-alert-close');
			...
			if ( alert['user_interaction'] == 'call_to_action' ) {
				...
				$alert_button.addClass( 'close-button' ).attr( 'aria-label', GJH_Alerts.closeButton );
			}
        ...
        if (alert['type'] == 'pop-up') {
            ...
            $alert.find('.gjh-alert-image.show-for-medium').append('<img src="' + alert['popup_image'] + '" />');
			
			if ( alert['popup_image_small'].length <= 0 ) {
				$alert.find('.gjh-alert-image.show-for-small-only').css( 'display', 'none' );
			}
			else {
				$alert.find('.gjh-alert-image.show-for-small-only').append('<img src="' + alert['popup_image_small'] + '" />');
			}
			
			if ( alert['user_interaction'] == 'call_to_action' ) {
			
				// Create an additional Button and place it as needed for a Close Button
				$alert.find( '.gjh-alert-content' ).append( $alert_button.first().clone().removeClass( 'call-to-action' ).addClass( 'close-button' ).addClass( 'gjh-alert-close' ).html( '' ).prepend('<span class="fa fa-times" aria-hidden="true"></span>').attr( 'href', '' ).attr( 'aria-label', GJH_Alerts.closeButton ) );
				...
				$alert_button.first().detach().appendTo( $alert.find( '.gjh-alert-content' ) );
    ...
    function close_popup($alert) {
        ...
        $alert.find('.gjh-alert-content').slideUp(400, function () {
	...
	function restore_focus() {
		...
		if ( typeof window.gjh_get_screen_size == 'function' && 
		   window.gjh_get_screen_size() == 'small' ) {
    ...
    window['gjh_clear_alert_cookies'] = function () {
